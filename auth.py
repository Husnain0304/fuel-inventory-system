...
def login_system(conn):
    ...
    with centre:
        ...
        if submitted and _login_allowed():
            ...
            if result:
                valid, needs_upgrade = verify_password(
                    password,
                    result[2],
                )
            else:
                valid, needs_upgrade = False, False

            if valid:
                if needs_upgrade:
                    cursor.execute(
                        "UPDATE users SET password = %s WHERE id = %s",
                        (hash_password(password), result[0]),
                    )
                    conn.commit()
...
def require_login(conn):
    if not st.session_state.get("user_id"):
        restored = _restore_session(conn)
    ...
    st.session_state["user"] = current[0]
    st.session_state["role"] = current[1]
